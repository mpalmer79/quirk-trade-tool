...
export class AuthService {
  ...
  private jwtExpiry: string = process.env.JWT_EXPIRY || '24h';
  private refreshExpiry: string = process.env.REFRESH_TOKEN_EXPIRY || '7d';
  ...
  generateTokens(user: User): { accessToken: string; refreshToken: string } {
    ...
    const refreshToken = jwt.sign(
      ...
      this.jwtSecret,
      {
        expiresIn: this.refreshExpiry as string | number,
        algorithm: 'HS256'
      } as jwt.SignOptions
    );
